...
export const testConnection = async () => {
  try {
    ...
    if (error.code === 'ER_ACCESS_DENIED_ERROR') {
      ...
      console.error(`      CREATE DATABASE ${process.env.DB_NAME || 'mwambo_store'};`);
    } else if (error.code === 'ECONNREFUSED') {
      console.error('   🔍 Database server is not running. Start MySQL/XAMPP first.');
    } else if (error.code === 'ENOTFOUND') {
      console.error('   🔍 Database host not found. Check DB_HOST in .env file.');
    }
    
    // General troubleshooting advice
    console.error('\n📋 Troubleshooting tips:');
    console.error('   1. Make sure MySQL/XAMPP is running');
    console.error('   2. Check your .env file has correct DB_* values');
    console.error('   3. Verify the database exists (create it if needed)');
    console.error('   4. For XAMPP, default credentials are:');
    console.error('      DB_HOST=localhost, DB_USER=root, DB_PASSWORD= (empty)');
