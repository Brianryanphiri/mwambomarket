...
const validateEnv = () => {
  ...
  for (const varInfo of requiredVars) {
    if (!process.env[varInfo.name] || process.env[varInfo.name].trim() === '') {
      missing.push(`${varInfo.name} (${varInfo.description})`);
    }
  }
  ...
  // Special check for JWT_SECRET length
  if (process.env.JWT_SECRET && process.env.JWT_SECRET.length < 32) {
    console.error('\n❌ CRITICAL ERROR: JWT_SECRET is too short');
    console.error('   For security, JWT_SECRET should be at least 32 characters long.');
...
if (!process.env.JWT_SECRET || process.env.JWT_SECRET.trim() === '') {
  ...
  console.error('Please set a JWT_SECRET in your .env file to secure authentication');
  console.error('Example: JWT_SECRET=your-super-secret-jwt-key-change-this\n');
  process.exit(1);
}
...
// Also serve root uploads for backward compatibility
  ...
  const tryPort = (port) => {
    const server = app.listen(port)
      ...
      .on('listening', () => {
        ...
        console.log(`🛡️  Security: Helmet enabled, Rate limiting active (100/15min, auth: 10/15min)`);
