import { 
  ...
  ShoppingBag, Tag, Sparkles, Award, Flame,
  Heart, Loader2
} from 'lucide-react';
...
const Header = () => {
  ...
  const categories = [
    ...
    { name: 'Household', icon: Home, slug: 'household' },
  ...
  return (
    <header className={`sticky top-0 z-50 bg-card/95 backdrop-blur-md border-b border-border transition-shadow ${scrolled ? 'shadow-md' : ''}`}>
      {/* Top bar */}
      ...
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center justify-between gap-4">
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0 group">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 flex items-center justify-center group-hover:scale-105 transition-transform">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            <Link to="/" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors">
              Home
            </Link>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors">
            ...
            <Link to="/shop" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors">
              Shop
            </Link>

            <Link to="/subscriptions" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors">
              Subscriptions
            </Link>

            <Link to="/services" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors">
              Services
            </Link>

            <Link to="/about" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors">
              About
            </Link>

            <Link to="/contact" className="px-3 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors">
              Contact
            </Link>
          </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
              <SheetTrigger asChild>
                <button className="relative flex items-center gap-2 bg-gradient-to-r from-orange-500 to-red-500 text-white px-4 py-2.5 rounded-xl font-medium text-sm transition-all hover:opacity-90">
                  <ShoppingCart className="w-4 h-4" />
                  <span className="hidden sm:inline">Cart</span>
                  {totalItems > 0 && (
                    <Badge className="absolute -top-2 -right-2 bg-amber-500 text-white border-none h-5 min-w-5 flex items-center justify-center text-xs animate-count-up">
                      {totalItems}
                    </Badge>
                  )}
                </button>
              ...
              <SheetContent className="w-full sm:max-w-md flex flex-col p-0">
                ...
                <div className="flex-1 overflow-y-auto py-4 px-6 space-y-3">
                  {items.length === 0 ? (
                    ...
                    items.map(item => (
                      <div key={item.productId} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 hover:bg-muted transition-colors">
                  ...
                  <div className="border-t border-border p-6 space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-amber-600">
                            MK {(10000 - subtotal).toLocaleString()} left
                          </span>
                        </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold">MK {subtotal.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground flex items-center gap-1.5">
                          <Truck className="w-3.5 h-3.5" />
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <Button 
                        ...
                        className="bg-gradient-to-r from-orange-500 to-red-500 text-white h-11 text-base font-semibold rounded-xl gap-2"
                      >
              ...
              return (
                <li key={cat.name}>
                  <Link
                    ...
                    className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors whitespace-nowrap inline-flex items-center gap-1.5"
                  >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-border bg-card animate-fade-in">
          <div className="container mx-auto px-4 py-3 space-y-1">
            ...
            <Link
              to="/"
              className="flex items-center gap-3 w-full text-left px-4 py-2.5 text-sm font-medium text-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors"
              ...
              {categories.map(cat => {
                ...
                return (
                  <Link
                    key={cat.name}
                    to={`/shop?category=${encodeURIComponent(cat.name)}`}
                    className="flex items-center gap-3 w-full text-left px-4 py-2.5 text-sm font-medium text-muted-foreground hover:text-primary hover:bg-muted rounded-lg transition-colors"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    <Icon className="w-4 h-4" />
                    {cat.name}
